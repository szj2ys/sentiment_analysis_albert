...
from modeling import AlbertModel, get_assignment_map_from_checkpoint
...
def read_csv(input_file):
    ...
    lines2 = [[
        list(hp.dict_label.keys())[list(hp.dict_label.values()).index(l[0])],
        l[1]
...
class DataProcessor(object):
    ...
    def _read_csv(cls, input_file):  # 项目
        ...
        lines2 = [[
            list(hp.dict_label.keys())[list(hp.dict_label.values()).index(
                l[0])], l[1]
        ] for l in lines if type(l[1]) == str]
        random.shuffle(lines2)
        return lines2
...
def _create_model_from_hub(hub_module, is_training, input_ids, input_mask,
    ...
    albert_inputs = dict(input_ids=input_ids,
                         input_mask=input_mask,
                         segment_ids=segment_ids)
    albert_outputs = albert_module(inputs=albert_inputs,
                                   signature="tokens",
                                   as_dict=True)
...
def _create_model_from_scratch(
    albert_config,
    is_training,
    input_ids,
    input_mask,
...
def create_model(
    ...
    if hub_module:
        ...
        output_layer = _create_model_from_scratch(
            albert_config,
            is_training,
            input_ids,
            input_mask,
