...
def cell_textcnn(inputs, is_training):
    ...
    with tf.name_scope("TextCNN"):
        for i, filter_size in enumerate(hp.filter_sizes):
            with tf.name_scope("conv-maxpool-%s" % filter_size):
                # Convolution Layer
                filter_shape = [
                    filter_size, hp.embedding_size, 1, hp.num_filters
                ]
                W = tf.Variable(
                ...
                b = tf.Variable(tf.constant(0.1, shape=[hp.num_filters]),
                                dtype=tf.float32,
                                name="b")
                conv = tf.nn.conv2d(inputs_expand,
                                    W,
                                    strides=[1, 1, 1, 1],
                                    padding="VALID",
                                    name="conv")
    ...
    # Dropout
    h_pool_flat_dropout = tf.nn.dropout(
        h_pool_flat, keep_prob=hp.keep_prob if is_training else 1)
