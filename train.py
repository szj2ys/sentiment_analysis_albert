...
MODEL_SAVE_PATH = hp.file_save_model
    ...
    for i in range(hp.num_train_epochs):
        ...
        for j in range(num_batchs - 1):
            ...
            if j % hp.summary_step == 0:
                summary, glolal_step = sess.run(
                    [MODEL.merged, MODEL.global_step], feed_dict=fd)
                writer.add_summary(summary, glolal_step)
            # Save Model
            if j % (num_batchs // hp.num_saved_per_epoch) == 0:
                if not os.path.exists(hp.file_save_model):
                    os.makedirs(hp.file_save_model)
                saver.save(
                    sess,
                    os.path.join(hp.file_save_model,
                                 "model_%s_%s.ckpt" % (str(i), str(j))),
                )
            # Log
            if j % hp.print_step == 0:
                ...
                print("Time:%s, Epoch:%s, Batch number:%s/%s, Loss:%s" %
                      (time_now_string(), str(i), str(j), str(num_batchs),
                       str(loss)))
                #  Loss of Test data
                ...
                print("Time:%s, Epoch:%s, Batch number:%s/%s, Loss(test):%s" %
                      (time_now_string(), str(i), str(j), str(num_batchs),
                       str(loss)))
