...
from modeling import AlbertConfig, AlbertModel, get_assignment_map_from_checkpoint
    ...
    def __init__(self, is_training):
        ...
        with tf.variable_scope("Prediction"):
            self.preds = tf.argmax(self.logits, axis=-1,
                                   output_type=tf.int32)[0] - 1
            self.preds_prob = self.probabilities[0][tf.argmax(
                self.probabilities, axis=-1)[0]]
            self.max_pb = self.preds, self.preds_prob
        # Summary for tensorboard
        with tf.variable_scope("Loss"):
            if self.is_training:
                self.accuracy = tf.reduce_mean(
                    tf.to_float(tf.equal(self.preds, self.label_ids)))
            ...
            if self.is_training:
                ...
                one_hot_labels = tf.one_hot(self.label_ids,
                                            depth=num_labels,
                                            dtype=tf.float32)
